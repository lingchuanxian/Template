...
public class LoadingWebView extends WebView{
    ...
    private void initContext(Context context) {
        ...
        getSettings().setDomStorageEnabled(false);//设置是否启用了DOM Storage API
        getSettings().setDatabaseEnabled(false);//开启database storage API功能
        //getSettings().setDatabasePath(cacheDirPath); //设置数据库缓存路径
        //getSettings().setAppCachePath(cacheDirPath);//设置Application Caches缓存目录
        getSettings().setAppCacheEnabled(false);//开启Application Caches功能
